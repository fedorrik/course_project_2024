#!/bin/bash
# Usage: ./count_all_kmers.sh <dit_with_fastas> <kmer length>
...
for name in `ls "kmers_$kmer_length/kmers_db"`; do
  echo "  $name"
  python3 get_specific_kmers.py $name "kmers_$kmer_length/kmers_db/" > "kmers_$kmer_length/kmers_specific_cnt/$name.cnt"
done
